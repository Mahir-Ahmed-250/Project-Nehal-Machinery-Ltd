...
import AdminSingleOther from "./AdminSingleOther";
...
const AdminOthers = () => {
  ...
  const onClickCreate = async () => {
    ...
    try {
      if (baseImage && serial && name && description) {
        ...
        swal(
          "Well Done!",
          "You have successfully Uploaded a Other!",
          "success",
          {
            buttons: {
              cancel: "Cancel",
  ...
  const [others, setRawMaterials] = useState([]);
  useEffect(() => {
    setLoading2(true);
    //create the query
    ...
    <>
      <>
        <div style={{ paddingTop: "12%" }} className="container">
          <BannerTitle title1="Current Others" />
          <AdminNavigation />
          <div className="row">
            {loading2 ? (
              <>
              ...
              <>
                {others
                  ...
                  .map((other) => (
                    <AdminSingleOther key={other.id} other={other} />
                  ))}
              </>
            )}
          </div>
          <div className="pb-5" style={{ marginTop: "180px" }}>
            <BannerTitle title1="Upload a new Others" />
            <div>
              <input
                type="number"
                id="form3Example3"
